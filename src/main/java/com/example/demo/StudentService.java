...
class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        //TESTIRUNEJA
        studentService.CreateStudent("Matti", "Meikäläinen");
        studentService.CreateStudent("Maija", "Mehiläinen");
        studentService.CreateOnlineCourse("J102354", "Javan perusteet", "Joopas Juupas", "01.09.2022", "21.10.2022", 15, "zoom/link", "Tässä kurssissa opetellaan javaa");
        studentService.CreateClassRoomCourse("C012354", "C++ perusteet", "Sussu Sössi", "06.09.2022", "20.10.2022", 15, "5B303", "Tämä on C++ kurssi");
        studentService.AddStudentToCourse(studentService.GetAllStudents().get(0), studentService.GetAllCourses().get(0));
        studentService.AddStudentToCourse(studentService.GetAllStudents().get(1), studentService.GetAllCourses().get(1));
        
        for(Student student : studentService.GetAllStudents()){
            System.out.println("Löytyy oppilas " + student.GetID());
        }

        fileService.SaveCourseInfo(studentService.GetAllCourses());
        fileService.SaveStudentInfo(studentService.GetAllStudents());

        studentService.SetLoadedCourses(fileService.LoadCourseInfo());
        studentService.SetLoadedStudents(fileService.LoadStudentInfo());

        System.out.println(studentService.GetAllCourses().size() + " size");

        for(Course course : studentService.GetAllCourses()){
            System.out.println("Löytyy kurssi" + course.GetName());
        }
        for(Student student : studentService.GetAllStudents()){
            System.out.println("Löytyy " + student.GetFirstName());
        }
    }
}
